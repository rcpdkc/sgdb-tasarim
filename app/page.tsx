  ...
  return <main className="page-shell">
    ...
    <form className="selection-form" onSubmit={submit}>
      ...
      <section className="product-section">
        ...
        <div className="design-grid polar-grid">{models.map((model) => <article key={model} className={`design-card ${form.polar_design === model ? "selected" : ""}`}><button type="button" className="image-button polar-preview" style={{ backgroundImage: `url(/images/web/polar-${model}.webp)` }} onClick={() => setModal({ kind: "polar", model })} aria-label={`Polar ${model} ön ve arka görünüşünü büyüt`}><span className="side-label front">ÖN</span><span className="side-label back">ARKA</span><span className="zoom-hint">Büyüt</span></button><button type="button" className="select-design" onClick={() => update("polar_design", model)}><strong>Polar {model}</strong><em>{form.polar_design === model ? "Seçildi" : "Seç"}</em></button></article>)}</div>
    ...
    {modal && <div className="modal-backdrop" role="presentation" onMouseDown={() => setModal(null)}><section className="image-modal" role="dialog" aria-modal="true" aria-label={`${modal.kind === "tshirt" ? "Tişört" : "Polar"} ${modal.model} büyük görünüş`} onMouseDown={(event) => event.stopPropagation()}><header><div><span>MODEL {modal.model}</span><h2>{modal.kind === "tshirt" ? "Tişört" : "Polar"} ön ve arka görünüş</h2></div><button type="button" onClick={() => setModal(null)} aria-label="Pencereyi kapat">×</button></header><div className="modal-image" role="img" aria-label={`${modal.kind === "tshirt" ? "Tişört" : "Polar"} ${modal.model} tam görünüş`} style={{ backgroundImage: `url(/images/web/${modal.kind === "tshirt" ? "tshirt" : "polar"}-${modal.model}.webp)` }}><span className="modal-side modal-front">ÖN</span><span className="modal-side modal-back">ARKA</span></div><button type="button" className="modal-select" onClick={() => { update(modal.kind === "tshirt" ? "tshirt_design" : "polar_design", modal.model); setModal(null); }}>Bu modeli seç</button></section></div>}
  </main>;
}
