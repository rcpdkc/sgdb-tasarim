...
export default function HomePage() {
  ...
  return <main className="page-shell">
    ...
    {modal && <div className="modal-backdrop" role="presentation" onMouseDown={() => setModal(null)}><section className="image-modal" role="dialog" aria-modal="true" aria-label={`${modal.kind === "tshirt" ? "Tişört" : "Polar"} ${modal.model} büyük görünüş`} onMouseDown={(event) => event.stopPropagation()}><header><div><span>MODEL {modal.model}</span><h2>{modal.kind === "tshirt" ? "Tişört" : "Polar"} ön ve arka görünüş</h2></div><button type="button" onClick={() => setModal(null)} aria-label="Pencereyi kapat">×</button></header>{modal.kind === "tshirt" ? <div className="modal-image" role="img" aria-label={`Tişört ${modal.model} tam görünüş`} style={{ backgroundImage: `url(/images/tshirt-${modal.model}.png?v=3)` }}><span className="modal-side modal-front">ÖN</span><span className="modal-side modal-back">ARKA</span></div> : <div className="modal-polar"><img src="/images/polar-catalog.png?v=3" alt={`Polar ${modal.model} ön ve arka görünüş`} style={{ transform: `translateX(-${(modal.model - 1) * 20}%)` }} /><span className="modal-side modal-front">ÖN</span><span className="modal-side modal-back">ARKA</span></div>}<button type="button" className="modal-select" onClick={() => { update(modal.kind === "tshirt" ? "tshirt_design" : "polar_design", modal.model); setModal(null); }}>Bu modeli seç</button></section></div>}
  </main>;
}
