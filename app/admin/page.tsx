...
export default function AdminPage() {
  ...
  async function load(pass = password) {
    ...
    sessionStorage.setItem("sgdb-admin", pass); setOrders(data); setLoading(false);
  }
  ...
  return <main className="admin-shell">
    ...
    {!orders.length && !loading ? <section className="login-card"><h2>Yönetici girişi</h2><p>Talep kayıtlarını görmek için yönetici şifresini girin.</p><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && load()} placeholder="Yönetici şifresi" /><button onClick={() => load()}>Giriş yap</button>{error && <p className="notice error">{error}</p>}</section> : <>
      ...
      <div className="table-wrap"><table><thead><tr><th>Kullanıcı</th><th>Ürün</th><th>Birim</th><th>Seçimler</th><th>Adet</th><th>Tarih</th><th>Durum</th></tr></thead><tbody>{shown.map((order) => <tr key={order.id}><td><strong>{order.full_name}</strong>{order.note && <small>{order.note}</small>}</td><td>{order.product}</td><td>{order.department}</td><td>{order.color} · {order.size} · Model {order.design_variant}</td><td>{order.quantity}</td><td>{order.created_at ? new Date(order.created_at).toLocaleString("tr-TR") : "—"}</td><td><select value={order.status} onChange={(e) => changeStatus(order.id!, e.target.value as OrderStatus)}>{statuses.map((s) => <option key={s}>{s}</option>)}</select></td></tr>)}</tbody></table>{!shown.length && <div className="empty">Bu filtrede kayıt bulunmuyor.</div>}</div>
    </>}
  </main>;
}
